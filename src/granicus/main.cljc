(ns granicus.main
  "Kotodama WASM entrypoint for the Granicus clean-room actor (L4) — Clojure port.
  ...
  facts (`granicus.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "granicus")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"granicus.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "granicus-compat" :tier tier :entities entities} 200])
...
(defn start! [] :granicus-compat/ready)
